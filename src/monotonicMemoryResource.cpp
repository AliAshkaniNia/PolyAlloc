...
memory::MonotonicMemoryResource::MonotonicMemoryResource(void* buffer, std::size_t buffer_size, std::pmr::memory_resource* upstream)
    : upstream_(upstream), buffer_(static_cast<char*>(buffer)), buffer_size_(buffer_size), offset_(0) {}


auto memory::MonotonicMemoryResource::do_allocate(std::size_t bytes, std::size_t alignment) -> void* {
...
void memory::MonotonicMemoryResource::do_deallocate(void* ptr, std::size_t bytes, std::size_t alignment) {
...
auto memory::MonotonicMemoryResource::do_is_equal(const std::pmr::memory_resource& other) const noexcept -> bool  {
    return this == &other;
}